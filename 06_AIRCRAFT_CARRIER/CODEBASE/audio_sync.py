...
class AudioSync:
    ...
    def auto_sync_duration(
        ...
        """Ajuste automatiquement l'audio à la durée calculée (total_frames / fps).

        Trim si trop long, pad si trop court. Fade out de 2s si trim.

        Args:
            audio_path: Fichier audio source
            total_frames: Nombre total de frames vidéo
            fps: Framerate vidéo
        ...
        if fps <= 0 or total_frames <= 0:
            self._log("Paramètres invalides pour auto_sync_duration")
            return False

        video_duration = total_frames / fps
        self._log(f"Auto-sync: {total_frames} frames / {fps} fps = {video_duration:.2f}s")

        return self.sync_to_video(audio_path, video_duration, output_path, fade_out_duration)
