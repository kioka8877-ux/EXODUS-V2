...
def create_link(source_dir: str, target_in_dir: str) -> Path:
    ...
    if not source.is_dir():
        raise FileNotFoundError(f"Source introuvable ou n'est pas un dossier : {source_dir}")
...
def resolve_input(in_dir) -> Path:
    ...
    try:
        ...
        if not source:
            print(f"[PHANTOM:WARN] _LINK.json sans champ 'source' dans {in_dir}, fallback sur {in_dir}")
            return in_path
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tgt = Path(tmpdir) / "target_in"
        tgt.mkdir()
        link_data = {"source": "/nonexistent/path/12345", "created": "2026-01-01T00:00:00", "created_by": "TEST"}
        ...
        passed += 1
        print(f"[PHANTOM:TEST] {passed}/{total} — resolve_input avec source invalide (fallback) OK")
